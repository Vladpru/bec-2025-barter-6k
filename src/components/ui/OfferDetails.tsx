      ...
      >
        ...
        <div className="overflow-y-auto custom-scroll-md max-h-[calc(80vh-80px)] 700px:max-h-[calc(90vh-100px)] pr-2">
          <div className="text-white space-y-8">
            {Array.isArray(text) && text.map((desc, index) => (
              <div key={index} className="space-y-1">
                <h3 className="text-xl 700px:text-2xl font-bold leading-[1.3]">{desc.title}</h3>
                <hr />
                <p className="text-sm 700px:text-base font-extralight leading-[1.6]">{desc.text}</p>
              </div>
            ))}
          </div>
        </div>
